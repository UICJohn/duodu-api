class Users::PasswordsController < Devise::PasswordsController
  ...
  def send_verify_code
    if params[:phone].present? && User.find_by(phone: params[:phone])
  ...
  def reset_by_phone
    return { 'error' => '手机号码不存在' } unless (resource = User.find_by(phone: params[:user][:phone]))
  ...
  def reset_by_email
    ...
    successfully_sent?(resource) ? { 'error' => '邮箱不正确' } : { 'message' => '验证邮件已发送' }
  end
end
